...
class PlotMenuInterface:
    def drawplot(self, dane, nazwa="Wykres", osx="X", osy="Y"):
        pass

    def drawscatter(self, dane: list, nazwa="Wykres"):
        pass

    def draw_groups_scatter(self, dane: dict):
        pass
...
class ButtonMenu(QWidget, MenuInterface, PlotMenuInterface):
    ...
    def draw_groups_scatter(self, dane: dict):
        for key in dane.keys():
            C = np.array([next(color_generator())] * len(dane.get(key)))
            plt.scatter(x=[float(element[0]) for element in dane.get(key)],
                        y=[float(element[1]) for element in dane.get(key)],
                        c=C / 255.0)
        plt.show()

    def drawscatter(self, dane: list, nazwa="Wykres"):
        C = np.array([next(color_generator())] * len(dane))
        plt.scatter(x=[float(element[0]) for element in dane], y=[float(element[1]) for element in dane],
                    c=C / 255.0)
        plt.show()

    def drawtable(self, dane: dict, legenda="Brak legendy", nazwa="Tabelka", prefixx="", prefixy=""):
        ...
        for i, el in enumerate(dane):
            self.tabelka.setItem(i + 1, 0, QTableWidgetItem(prefixx + str(el)))
            self.tabelka.setItem(0, i + 1, QTableWidgetItem(prefixy + str(el)))
